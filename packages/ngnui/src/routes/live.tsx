...
import { LuPlay, LuTerminal, LuTriangleAlert } from "react-icons/lu";
...
function RouteComponent() {
  ...
  // The UI reads a database file; it cannot execute anything itself. Running
  // needs an `ngn run` process to forward to, so ask whether there is one
  // before offering a Run button that could only fail.
  const { data: meta } = useQuery({
    queryKey: ["meta"],
    queryFn: fetchMeta,
    // Cheap, and a run started after the page was opened should light this up.
    refetchInterval: 15_000,
  });
  const liveAvailable = meta?.live.available ?? false;
...
declare module "@apisurf/ngn" {
  export interface TaskContext {
    ...
    meta: {
      ...
      /** File descriptor with path and filename info */
      file: {
        type: "file";
        path: {
          absolute: string;
          ...
          <HStack gap={3} align="flex-start">
            ...
            <Box>
              ...
              <Text fontSize="xs" color="fg.muted">
                ngnui reads a database file and cannot execute tasks itself.
                Start{" "}
                <Text as="span" fontFamily="mono">
                  ngn run
                </Text>{" "}
                and restart ngnui with{" "}
                <Text as="span" fontFamily="mono">
                  --live http://127.0.0.1:4545
                </Text>
                {meta.live.url
                  ? ` — nothing answered at ${meta.live.url}.`
                  : "."}
              </Text>
            </Box>
          </HStack>
        ...
        <Flex align="center" justify="space-between" gap={6} flexWrap="wrap">
          ...
          <HStack gap={2}>
            <Button
              ...
              color="fg.emphasized"
              size="sm"
              disabled={isRunning || !liveAvailable}
            >
              <LuPlay size={14} />

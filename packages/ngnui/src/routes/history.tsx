...
function RouteComponent() {
  ...
  // Deliberately keyed on the debounced term alone: this effect exists only to
  // push a settled search term into the URL. `status` is already written by the
  // filter handlers, and `handleSearch` is re-created every render, so widening
  // the deps would re-navigate on unrelated renders — including restoring a
  // stale term after a tab change, since the debounce lags the reset.

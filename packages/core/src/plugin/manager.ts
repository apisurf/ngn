import type { Plugin, PluginInitContext } from "./types.js";

export interface PluginManagerOptions {
  plugins: Plugin[];
  initContext: PluginInitContext;
}
...
export class PluginManager {
  ...
  private initContext: PluginInitContext;
  private initializedApis: Map<string, unknown> = new Map();
  private isInitialized = false;

  constructor(options: PluginManagerOptions) {
    this.plugins = options.plugins ?? [];
    this.initContext = options.initContext;
  ...
  async init(): Promise<void> {
    ...
    for (const plugin of this.plugins) {
      try {
        const api = await plugin.init(this.initContext);
